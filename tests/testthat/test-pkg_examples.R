...
.test_multithreaded <- function() {
  expect_error(RcppProgress:::test_multithreaded(nb = 1000, threads = 4), NA)
}
...
.amardillo_multithreaded <- function() {
  expect_error(RcppProgress:::amardillo_multithreaded(nb = 1000, threads = 4), NA)
}
...
.eta_progress_bar <- function() {
  expect_error(RcppProgress:::eta_progress_bar(nb = 1000), NA)
}
